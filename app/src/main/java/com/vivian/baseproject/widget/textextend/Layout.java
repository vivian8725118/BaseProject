...
public abstract class Layout {
	 ...
	 */
	/* package */void replaceWith(CharSequence text, TextPaint paint, int width, Alignment align, float spacingmult, float spacingadd) {
		if (width < 0) {
			throw new IllegalArgumentException("Layout: " + width + " < 0");
		}
				...
				for (int n = 0; n < length; n++) {
					if (spans[n] instanceof LeadingMarginSpan) {
						...
						if (dir == DIR_RIGHT_TO_LEFT) {
							margin.drawLeadingMargin(c, paint, right, dir, ltop, lbaseline, lbottom, buf, start, end, isFirstParaLine, null);

							right -= margin.getLeadingMargin(isFirstParaLine);
						} else {
							margin.drawLeadingMargin(c, paint, left, dir, ltop, lbaseline, lbottom, buf, start, end, isFirstParaLine, null);
	...
	public void getCursorPath(int point, Path dest, CharSequence editingBuffer) {
		...
		int caps = TextKeyListener.getMetaState(editingBuffer, KeyEvent.META_SHIFT_ON) | TextKeyListener.getMetaState(editingBuffer, 0x800);
	...
	private void drawText(Canvas canvas, CharSequence text, int start, int end, int dir, Directions directions, float x, int top, int y, int bottom, TextPaint paint, TextPaint workPaint,
	...
	private static float measureText(TextPaint paint, TextPaint workPaint, CharSequence text, int start, int offset, int end, int dir, Directions directions, boolean trailing, boolean alt,
	 ...
	 *
	 * @param paint
	 *            the paint for the text; will not be modified
	 * @param workPaint
	 *            paint available for modification
	 * @param text
	 *            text
	 * @param start
	 *            start of the line
	 ...
	 *            limit of the line
	 * @param fm
	 *            object to return integer metrics in, can be null
	 * @param hasTabs
	 *            true if it is known that the line has tabs
	 * @param tabs
	 *            tab position information
	 * @return the width of the text from start to end
	 */
	/* package */static float measureText(TextPaint paint, TextPaint workPaint, CharSequence text, int start, int end, Paint.FontMetricsInt fm, boolean hasTabs, Object[] tabs) {
		char[] buf = null;
	 ...
	 */
	/* package */static float nextTab(CharSequence text, int start, int end, float h, Object[] tabs) {
