 ...
 * like {@link Spanned} object with {@link ReplacementSpan}.
 *
 * @hide
 */
public class Styled {
	 ...
	 * <p>
	 * The x position is the leading edge of the text. In a right-to-left paragraph, this will be to the right of the text to be drawn. Paint should not have an Align value other than LEFT or
	 * positioning will get confused.
	 * <p>
	 * On return, workPaint will reflect the original paint plus any modifications made by character styles on the run.
	 * <p>
	 * The returned width is signed and will be < 0 if the paragraph direction is right-to-left.
	 */
	private static float drawUniformRun(Canvas canvas, Spanned text, int start, int end, int dir, boolean runIsRtl, float x, int top, int y, int bottom, Paint.FontMetricsInt fmi, TextPaint paint,
		...
		if (replacement == null) {
			...
			if (canvas != null) {
				...
				if (dir == Layout.DIR_RIGHT_TO_LEFT) {
					...
					canvas.drawText(tmp, tmpstart, tmpend, x, y + workPaint.baselineShift - (bottom - top - workPaint.getTextSize()) / 2, workPaint);
				}
			} else {
	 ...
	 * @param workPaint
	 *            a paint to modify; on return will reflect the original
	 ...
	 * <p>
	 * The returned advance width will be < 0 if the paragraph direction is right-to-left.
	 */
	private static float drawDirectionalRun(Canvas canvas, CharSequence text, int start, int end, int dir, boolean runIsRtl, float x, int top, int y, int bottom, Paint.FontMetricsInt fmi,
	 ...
	 */
	/* package */static float drawText(Canvas canvas, CharSequence text, int start, int end, int dir, boolean runIsRtl, float x, int top, int y, int bottom, TextPaint paint, TextPaint workPaint,
			boolean needWidth) {
		// XXX this logic is (dir == DIR_LEFT_TO_RIGHT) == runIsRtl
		if ((dir == Layout.DIR_RIGHT_TO_LEFT && !runIsRtl) || (runIsRtl && dir == Layout.DIR_LEFT_TO_RIGHT)) {
			// TODO: this needs the real direction
			float ch = drawDirectionalRun(null, text, start, end, Layout.DIR_LEFT_TO_RIGHT, false, 0, 0, 0, 0, null, paint, workPaint, true);
	 ...
	 * draws the text). See also {@link Canvas#drawText(CharSequence, int, int, float, float, Paint)} and
	 * {@link Canvas#drawRect(float, float, float, float, Paint)}.
	 *
	 * @param canvas
	 *            The target canvas
	 * @param text
	 ...
	 * @param direction
	 *            The direction of the text. This must be {@link android.text.Layout#DIR_LEFT_TO_RIGHT} or {@link android.text.Layout#DIR_RIGHT_TO_LEFT}.
	 * @param x
	 ...
	 * @param paint
	 *            The main {@link TextPaint} object.
	 * @param workPaint
	 *            The {@link TextPaint} object used for temporal
	 *            workspace.
	 * @param needWidth
	 *            If true, this method returns the width of drawn text
	 ...
	 *
	 * @param paint
	 *            the main {@link TextPaint} object; will not be modified
	 * @param workPaint
	 *            the {@link TextPaint} object available for modification;
	...
	public static float measureText(TextPaint paint, TextPaint workPaint, CharSequence text, int start, int end, Paint.FontMetricsInt fmi) {
		return drawDirectionalRun(null, text, start, end, Layout.DIR_LEFT_TO_RIGHT, false, 0, 0, 0, 0, fmi, paint, workPaint, true);
	}
}